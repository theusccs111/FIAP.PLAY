...
namespace FIAP.PLAY.Tests.Application.UserAccess.Validations
{
    public class UserValidatorRequestTests
    {
        ...
        {
            var model = new UserRequest(string.Empty, "Has@!12356", "jaque@email.com",  ERole.Admin, true);
        ...
        {
            var model = new UserRequest(nome, "Has@!12356", "jaque@email.com",  ERole.Admin, true);
            var result = _validator.TestValidate(model);
            result.ShouldHaveValidationErrorFor(user => user.Name);
        }
        ...
        {
            var model = new UserRequest("Jacqueline", "Has@!12356", "email-invalido", ERole.Admin, true);
            var result = _validator.TestValidate(model);
        ...
        {
            var model = new UserRequest("Jacqueline", string.Empty, "jaque@email.com",  ERole.Admin, true);
            var result = _validator.TestValidate(model);
        ...
        {
            var model = new UserRequest("Jacqueline", senha, "jaque@email.com",  ERole.Admin, true);
            var result = _validator.TestValidate(model);
            result.ShouldHaveValidationErrorFor(user => user.PasswordHash);
        }
        ...
        {
            var model = new UserRequest("Jacqueline", "Has@!12356", "jaque@email.com",  ERole.Admin, true);
